...
save_plot(fig1, "jump_density_hongkong_distribution_signed", format='pdf')
plt.close(fig1)
...
save_plot(fig2, "jump_density_hongkong_distribution_abs", format='pdf')
plt.close(fig2)
...
save_plot(fig2b, "jump_density_hongkong_distribution_tail", format='pdf')
plt.close(fig2b)
...
save_plot(fig3, "jump_density_hongkong_qq_plot", format='pdf')
plt.close(fig3)
...
# %%
# Visualize examples from dataset and show actual price data
if X_windows is not None and jumps_subset is not None:
    ...
    if len(endogenous_jumps) > 0 and len(exogenous_jumps) > 0:
        ...
        plt.tight_layout()
        save_plot(fig_examples, "jump_density_hongkong_examples", format='pdf')
        ...
        endogenous_jumps2 = jumps_subset[jumps_subset["classification"] == "Endogenous"]
        exogenous_jumps2 = jumps_subset[jumps_subset["classification"] == "Exogenous"]
        
        if len(endogenous_jumps2) > 0 and len(exogenous_jumps2) > 0:
            endo_example2 = endogenous_jumps2.iloc[np.abs(endogenous_jumps2["D1_reflexivity"] - endogenous_jumps2["D1_reflexivity"].median()).argmin()]
            exo_example2 = exogenous_jumps2.iloc[np.abs(exogenous_jumps2["D1_reflexivity"] - exogenous_jumps2["D1_reflexivity"].median()).argmin()]
            examples2 = [
                (endo_example2, "Endogenous", "blue"),
                (exo_example2, "Exogenous", "red")
            ]
            
            fig_prices, axes = plt.subplots(2, 2, figsize=(14, 10))
            fig_prices.suptitle(f"Real Examples from Dataset: Actual Price & Returns\nEndogenous: {endo_example2['ticker']} at {endo_example2['timestamp']} | Exogenous: {exo_example2['ticker']} at {exo_example2['timestamp']}", 
                                fontsize=12)
            
            for col_idx, (ex, label, color) in enumerate(examples2):
                ticker = ex['ticker']
                ts = ex['timestamp']
            ...
            plt.tight_layout()
            save_plot(fig_prices, "jump_density_hongkong_examples_prices", format='pdf')
        ...
        save_plot(fig_dist, "jump_density_hongkong_D1_classification", format='pdf')
        plt.close(fig_dist)
