    ...
    if len(endogenous_jumps) > 0 and len(exogenous_jumps) > 0:
        ...
        if len(endogenous_jumps2) > 0 and len(exogenous_jumps2) > 0:
            ...
            fig_prices = make_subplots(
                rows=2, cols=2,
                subplot_titles=(
                    f"Endogenous: {endo_example2['ticker']} - Price",
                    f"Exogenous: {exo_example2['ticker']} - Price",
                    f"Endogenous: {endo_example2['ticker']} - Returns",
                    f"Exogenous: {exo_example2['ticker']} - Returns"
                ),
            vertical_spacing=0.12,
                horizontal_spacing=0.1
            )
            
            for col_idx, (ex, label, color) in enumerate(examples2, 1):
                ticker = ex['ticker']
                ts = ex['timestamp']
            ...
            fig_prices.update_layout(
                title=f"Real Examples from Dataset: Actual Price & Returns<br>Endogenous: {endo_example2['ticker']} at {endo_example2['timestamp']} | Exogenous: {exo_example2['ticker']} at {exo_example2['timestamp']}",
                template="plotly_white",
                height=700,
                showlegend=False
            )
