...
For each frequency cluster, we compute the energy signal $E(t)$ by integrating over the frequency bins in that cluster. 
Since DTMF signals are dual-tone (simultaneous low and high frequencies), we apply changepoint detection on each frequency bin within a cluster and select the bin with the minimum number of changepoints as the representative (the dual-tone nature makes one frequency more stable). 
We use the PELT algorithm with BIC penalty $\beta = 2\sigma^2 \log(T)$, where $\sigma^2$ is estimated from the signal variance and $T$ is the signal length.

\textbf{Step 5: Period Extraction and Filtering.} 
We combine all changepoints from all frequency clusters, order them chronologically, and extract time periods between consecutive changepoints. 
We compute the distribution of gap durations (silences between symbols) and remove lower outliers using the IQR method, ensuring that the total duration of remaining periods exceeds $0.4$ seconds (a calibration threshold).

\textbf{Step 6: Period-Symbol Matching.} 
Given $N$ ground truth symbols, we select the $N$ largest periods by duration, order them chronologically, and match them one-to-one with the ground truth symbols in temporal order.

\textbf{Step 7: Frequency Pair Detection and Symbol Verification.} 
For each matched period, we identify the active low and high frequencies by checking which clusters have changepoints near the period boundaries (tolerance: 2000 samples) or by verifying energy levels above a threshold (ratio $0.1$ of maximum energy). 
We map the detected frequency pair to a DTMF symbol using the standard DTMF frequency table and compare it with the assigned ground truth symbol.

\textbf{Hyperparameters:} STFT window length $256$, overlap $128$, energy clustering $k=2$, frequency clustering $k=8$, BIC penalty coefficient $2\sigma^2 \log(T)$, minimum total period duration $0.4$ s, changepoint matching tolerance $2000$ samples, energy threshold ratio $0.1$.
